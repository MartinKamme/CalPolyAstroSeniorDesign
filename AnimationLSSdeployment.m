%% AERO 449 Martin Kamme a = 7500 km LSS deployment
...
% Re-initialize the variables for the LCV
rpert = R_lcv;
vpert = V_lcv;
rosc = R_lcv;
vosc = V_lcv;
...
for i =1:length(rplotLSS)
    [rHill(:,i),vHill(:,i)] = ECI2Hill_Vectorized(rplotLSS(:,i),vplotLSS(:,i),rplotLCV(:,i),vplotLCV(:,i));
    rHillplot(i) = 1000*norm(rHill(:,i));
    vHillplot(i) = 1000*norm(vHill(:,i));
end

% Plot the things----------------------------------------------------------
tplot = linspace(0,t,length(rplotLSS));
figure
hold on
grid on
plot(tplot,rHillplot)
title('Relative Distance after release')
xlabel('Time (seconds)')
ylabel('Distance between LCV and LSS (m)')
xlim([0 1800]) 
ylim([0 1.5])
[x,y,z] = sphere;
x = x*1731;
...
plot3(rplotLSS(1,:),rplotLSS(2,:),rplotLSS(3,:),'b')

for i = 1:length(rplotLSS)

        lcv = plot3(rplotLCV(1,i),rplotLCV(2,i),rplotLCV(3,i),'gs','MarkerFaceColor','green','markerSize',10);
        lss = plot3(rplotLSS(1,i),rplotLSS(2,i),rplotLSS(3,i),'rs','MarkerFaceColor','red','markerSize',6);

    drawnow
  if i < length(rplotLSS)
    delete(lss)
    delete(lcv)
  end
...
% Post Calculations--------------------------------------------------------
timeToWait = ((pi/4))/(n_lcv - n_op);
massProp = t*(T/(Isp*g0));
deltaV = 1000*abs(Isp*g0*log(1-(T/(m0*g0*Isp))*t));
